...
SELECT property_id, booking_count, ROW_NUMBER() OVER(ORDER BY booking_count) FROM (SELECT COUNT(*) AS  booking_count, property_id FROM bookings GROUP BY property_id) derived;
